...
START_TEST (test_with_table_name_null)
{
...
START_TEST (correct_table_input_for_first_table)
{
	struct storage_record record;
	strncpy(record.value, TABLE1_COLVAL, sizeof record.value);
	int status = storage_set(TABLE1, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_set with correct input type should work");
}
END_TEST

START_TEST (correct_table_input_for_second_table)
{
	struct storage_record record;
	strncpy(record.value, TABLE2_COLVAL, sizeof record.value);
	int status = storage_set(TABLE2, KEY, &record, test_conn);
	fail_unless(status == 0, "storage_set with correct input type should work");
}
END_TEST
