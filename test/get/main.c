...
{
	struct storage_record record;
	int status = storage_get("NONEXISTINGTABLE", KEY1, &record, test_conn);
...
START_TEST (table1_getting_correctly)
{
	struct storage_record record;
	int status = storage_get(TABLE1, KEY1, &record, test_conn);
	fail_unless(status == 0, "storage get function is not working correctly");
}
	
END_TEST

START_TEST (table2_getting_correctly)
{
	struct storage_record record;
	int status = storage_get(TABLE2, KEY1, &record, test_conn);
	fail_unless(status == 0, "storage get function is not working correctly");
}
END_TEST

START_TEST (table3_getting_correctly)
{
	struct storage_record record;
	int status = storage_get(TABLE3, KEY1, &record, test_conn);
	fail_unless(status == 0, "storage get function is not working correctly");
}	
END_TEST
...
int main(int argc, char *argv[])
{
	...
	tc = tcase_create("test_getinvalid_badtable");
	...
	tc = tcase_create("test_getmissing_missingtable");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, test_getmissing_missingtable);
	suite_add_tcase(s, tc);

	tc = tcase_create("table1_getting_correctly");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, table1_getting_correctly);
	suite_add_tcase(s, tc);

	tc = tcase_create("table2_getting_correctly");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, table2_getting_correctly);
	suite_add_tcase(s, tc);

	tc = tcase_create("table3_getting_correctly");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	tcase_add_test(tc, table3_getting_correctly);
	suite_add_tcase(s, tc);
